...
class BDD:
    ...
    def print_info(self, nvars, output_file=None):
        ...
        #     print(i, ': ', rank[i])
        return rank
    def write_info_to_file(self, nvars, output_file):
        rank = [[] for i in range(nvars + 1)]
        _, rank = self._print_info(0, rank, output_file)
...
class BDD_Compiler:
    ...
    def compile(self, key_type='cutset', output_file=None):
        bdd = self.cnf2obdd(self.clausal_form, 1, key_type)
        if output_file:
            bdd.write_info_to_file(self.n_vars, output_file)
        return bdd
...
# Assuming you have already defined the BDD and BDD_Compiler classes and copied the code
